  ...
  return (
    <div className={step >= 3 && isComplete ? 'block' : 'hidden'}>
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-10 shadow-2xl border border-emerald-200 mb-12">
        <h4 className="text-2xl font-bold text-gray-900 mb-8 text-center">Your Healing Session</h4>
        <div className="grid md:grid-cols-2 gap-8 text-lg">
          <div>
            <div className="flex items-center gap-3 mb-4 p-4 bg-emerald-50 rounded-2xl">
              <div className="w-12 h-12 bg-emerald-500 rounded-2xl flex items-center justify-center">
                <span className="text-white font-bold text-xl">{form.watch('treatmentName')?.slice(0, 1)}</span>
              </div>
              <div>
                <div className="font-bold text-gray-900">{form.watch('treatmentName')}</div>
                <div className="text-emerald-600 font-semibold">60 min • €60</div>
              </div>
            </div>
            <div className="flex items-center gap-3 p-4 bg-indigo-50 rounded-2xl">
              ...
              <div className="font-semibold text-gray-900">{form.watch('doctorName')}</div>
            </div>
          </div>
          <div>
            <div className="p-6 bg-gradient-to-r from-emerald-50 to-indigo-50 rounded-3xl border border-emerald-200">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="font-bold text-xl text-gray-900">{form.watch('timeSlot')} • {form.watch('date')?.toLocaleDateString()}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-6 max-w-2xl mx-auto">
        <h4 className="text-2xl font-bold text-emerald-700 text-center">Contact Details</h4>
